...
install_port_chain(struct ovn_datapath *od, struct hmap *lflows, struct hmap *ports)
{
   ...
   *     * Remove port-security on VNF Ports
   *
   */
  ...
  uint16_t egress_inner_priority = 100;
  uint16_t egress_outer_priority = 150;
  ...
  VLOG_INFO("Iterating through %u services \n", (unsigned int)od->nbs->n_port_chains);
  /*
  ...
  for (size_t i = 0; i < od->nbs->n_port_chains; i++) {
     ...
     */
    *input_port_array = xmalloc(sizeof(src_port) * od->nbs->port_chains[i]->n_port_pair_groups);
    *output_port_array = xmalloc(sizeof(dst_port) * od->nbs->port_chains[i]->n_port_pair_groups+1);
    /*
    ...
    for (size_t j = 0; j < od->nbs->port_chains[i]->n_port_pair_groups; j++){
       ...
       *
       *  for (size_t k = 0; k < od->port_chains[i]->port_pair_groups[j]->n_port_pairs; k++){
       */
      input_port_array[j] = ovn_port_find(ports,od->nbs->port_chains[i]->port_pair_groups[j]->port_pairs[0]->inport->name);
      VLOG_INFO("In port for port-pair-group %u : %s\n", (unsigned int)j ,input_port_array[j]->key);
      output_port_array[j] =  ovn_port_find(ports,od->nbs->port_chains[i]->port_pair_groups[j]->port_pairs[0]->outport->name);
      VLOG_INFO("Out port for port-pair-group %u : %s\n", (unsigned int)j ,output_port_array[j]->key);
    }

    /*
     * Get the flow classifier and build the match
     */
    fc = od->nbs->port_chains[i]->flow_classifier;
    ...
    if (fc->logical_source_port != NULL){
    ...
    if (fc->logical_destination_port !=NULL){
      dst_port =  ovn_port_find(ports,fc->logical_destination_port->name);
    }
    output_port_array[od->nbs->port_chains[i]->n_port_pair_groups+1] = src_port;
    /*
    ...
    */
     ...
     */
    for (size_t j = 0; j < od->nbs->port_chains[i]->n_port_pair_groups+1; j++){ 
      /*
      ...
      fc_match = xasprintf("ip4.dst == "IP_FMT" && inport == %s",		\
			   IP_ARGS(src_logical_port_ip),output_port_array[j]->json_key);
    ...
    if (bidirectional == true) {
       ...
       */
      for (size_t j =  od->nbs->port_chains[i]->n_port_pair_groups; j < od->nbs->port_chains[i]->n_port_pair_groups+1; j--) { 
	fc_match = xasprintf("ip4.dst == "IP_FMT" && inport == %s",	\
			     IP_ARGS(src_logical_port_ip),output_port_array[j]->json_key);
	pc_action = xasprintf("outport = %s;"" output;",input_port_array[j-1]->json_key);
	...
	if (od == op->od) {
	  ...
	  if (ovs_scan(op->nbs->addresses[0],  ETH_ADDR_SCAN_FMT" "IP_SCAN_FMT,
	    ...
	    ds_put_format(&service_match, "inport == %s && ip4.dst == "IP_FMT, \
			  input_port_array[0]->json_key,IP_ARGS(def_ip));
                    ...
                    struct hmap *lflows, struct hmap *mcgroups)
{
  ...
  HMAP_FOR_EACH(od, key_node, ports) {
    if (!od->nbs) {
      continue;
    }
